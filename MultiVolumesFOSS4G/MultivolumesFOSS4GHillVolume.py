#!/usr/bin/env python3
'''Generates a CSV file with a row for every graduation in the surface starting with the lowest value. 
Each row gives the area and volume of all the pixels above that graduation (inclusive).

Set the user defined parameters below  

Run in your IDE, an ArcGIS Pro Python Window, or a QGIS Python Window.'''
...
try:
    ...
    #output file.
    output_csv = r"C:\gTemp\pyramidtestnegativevalues.csv"
    #The user defined graduations.  The output csv will contain a row for every graduation starting with the minimum elevation value.
    graduations = 1
    ...
    while this_elevation <= ras_maximum:
        ...
        #this counts all of the pixels considered at any given graduation.
